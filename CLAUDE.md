...
- **Only the web port is configurable**: `.env`'s `WEB_PORT` (default 8081) feeds `${WEB_PORT:-8081}:80`; `sss.sh` reads it back via `env_get`/`web_port()` (grep, not `source`) for the "web 地址" line. The agent report port **stays hardcoded as `35601:35601`** — deliberately not exposed, since changing it would silently drop every already-installed agent (their systemd unit has no `PORT=`).
